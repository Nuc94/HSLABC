...
int main() {
    ...
    std::vector< std::unique_ptr< rouflex::ExecTS > > execs;
    ...
    for(int mk = 1; mk <= N_MK; ++mk) {
        ...
        for(int run = 1; run <= N_RUNS; ++run) {
            run_path = mk_path + "/TS/Run" + std::to_string(run);
            std::cout << run_path << std::endl;
            execs.emplace_back( std::make_unique<rouflex::ExecTS>(*instances.back(), run_path, seed) );
            ++seed;
        }
    }
    ...
    for(int t = 0; t < n_threads - 1; ++t) {
        ...
        threads.emplace_back( std::thread(rouflex::ts_parallel_exec, std::ref(execs), std::ref(global_read_point)) );
    }
    rouflex::ts_parallel_exec( execs, global_read_point );
